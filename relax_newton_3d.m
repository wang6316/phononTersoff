...
xk=zeros(3*na,1);%position vector on the run
Hkinv=eye(3*na);%initialize inverse of the Hesian matrix as identity
sk=zeros(3*na,1);%virtual displacement of current step
gradxk=zeros(3*na,1);
gradxk1=zeros(3*na,1);
qk=zeros(3*na,1);%difference in gradient
...
parfor i=1:(3*na) %runs over na atoms %runs over x,y,z
    xktemp1=xk;
    ...
    gradxk(i)=(Etotal(xktemp1,xlatt,ylatt,zlatt,na,type)-Etotal(xktemp2,xlatt,ylatt,zlatt,na,type))/(2*0.01*a);
end

Ek=Etotal(xk,xlatt,ylatt,zlatt,na,type);
...
parfor i=1:(3*na)
    ...
    gradxk1(i)=(Etotal(xk1temp1,xlatt,ylatt,zlatt,na,type)-Etotal(xk1temp2,xlatt,ylatt,zlatt,na,type))/(2*0.01*a);
end

Ek1=Etotal(xk1,xlatt,ylatt,zlatt,na,type);
